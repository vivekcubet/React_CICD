import {useEffect, useState} from 'react';

import {useToast} from 'react-native-toast-notifications';
import {END_POINTS} from '../../../constants';
import useArchiveApi from '../../useArchiveApi';
import useGetAttachments from './useGetAttachments';
import usePostApi from '../../usePostApi';

export interface ArchiveAttachmentInterface {
  id: number;
  isArchive: boolean;
}

interface ArchiveResponse {
  data: any;
  // Define the expected response properties here
}

const useArchiveAttachment = (): [
  (data: ArchiveAttachmentInterface) => Promise<ArchiveResponse>,
  {errors: any | null},
] => {
  const [getAttachments] = useGetAttachments();
  const toast = useToast();
  const [errors, setError] = useState<any | null>(null);
  const [archiveData, {error}] = useArchiveApi();
  const [postApi] = usePostApi();
  useEffect(() => {
    setError(error);
  }, [error]);
  const archiveAttachment = async (data: ArchiveAttachmentInterface) => {
    console.log(data, 'ARCHIVE DATA=====');
    const response = data.isArchive
      ? await archiveData({
          endPoint:
            END_POINTS.ARCHIVE_ATTACHMENT + '?id=' + data?.id?.toString(),
          params: {id: data?.id?.toString()},
        })
      : await postApi({
          endPoint: END_POINTS.UNARCHIVE_ATTACHMENT,
          params: {id: data?.id?.toString()},
        });

    if (response) {
      console.log(response, 'TEXT=========');
      getAttachments({isLoader: true});
      toast.show(response.message);
      return response.message;
    }
  };

  return [archiveAttachment, {errors}];
};

export default useArchiveAttachment;
