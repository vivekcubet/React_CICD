...
} from '../../../redux/reducers/ServiceTaskReducer';
...
const useGetServiceTasks = (): [(options: GetData) => Promise<GetResponse>] => {
  const {roleType, userCompany} = useSelector(
    (state: RootState) => state.AuthReducer,
  );
  const dispatch = useDispatch();
  const [get] = useGetApi();
  const getTaskList = async (options: GetData) => {
    ...
    if (
      ...
      if (response.data) {
        ...
        const updateAction =
          roleType === 'sAdmin'
            ? updateMyServiceTasks(Object.values(data))
            : updatePublicServiceTasks(Object.values(data));
        ...
        */
        ...
        if (archivedResponse.data) {
          dispatch(
            updateArchivedServiceTasks(Object.values(archivedResponse?.data)),
          );
        }
        if (roleType !== 'sAdmin') {
          ...
          if (myCatResponse.data) {
            ...
            dispatch(updateMyServiceTasks(Object.values(myCatalogData)));
          }
        }
...
export default useGetServiceTasks;
